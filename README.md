...
_A WordPress plugin to sync content with a GitHub repository (or Jekyll site)_
...
Note that WordPress <--> GitHub Sync will _only_ import posts from the `master` branch. Changes on other branches will be ignored.
...
-   `target=`
    -   `'view'` (default) the url used will be the _view_ URL (`/blob/`).
    -   `'edit'` the url used will be the _edit_ URL (`/edit/`).
-   `type=`
